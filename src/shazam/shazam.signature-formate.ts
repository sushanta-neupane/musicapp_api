...
export class FrequencyPeak {
  constructor(
    public fftPassNumber: number,
    public peakMagnitude: number,
    public correctedPeakFrequencyBin: number,
...
export class DecodedMessage {
  ...
  static decodeFromBinary(bytes: Uint8Array) {
    ...
    let ptr = 0;
    const read = (e?: number) =>
      e === undefined ? bytes.slice(ptr, (ptr = bytes.length)) : bytes.slice(ptr, (ptr += e));
    ...
    self.numberSamples = Math.round(
      header.numberSamplesPlusDividedSampleRate - self.sampleRateHz * 0.24
    );
  ...
  encodeToBinary() {
    ...
    for (let x of Object.entries(this.frequencyBandToSoundPeaks)
      .map((a) => [FrequencyBand[a[0] as any] as unknown as number, a[1]])
      .sort((a, b) => (a[0] as number) - (b[0] as number))) {
      const frequencyBand = x[0] as number,
